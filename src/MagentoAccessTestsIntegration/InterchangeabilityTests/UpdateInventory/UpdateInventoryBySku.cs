...
namespace MagentoAccessTestsIntegration.InterchangeabilityTests.UpdateInventory
{
	...
	internal class InterchangeabilityTests_UpdateInventoryBySku : BaseTest
	{
		...
		public void UpdateInventoryBySku( MagentoServiceSoapCredentials credentialsRest, MagentoServiceSoapCredentials credentialsSoap )
		{
			...
			// ------------ Act
			var getProductsTaskRest = magentoServiceRest.GetProductsAsync( new[] { 0, 1 }, skus : skus, includeDetails : false );
			getProductsTaskRest.Wait();
			var inventoryToUpdate = getProductsTaskRest.Result.Where( p => p.ProductType == "simple" ).OrderBy( p => p.ProductId ).Select( p => new InventoryBySku() { Sku = p.Sku, Qty = long.Parse( p.Qty ) + 1 } );
			magentoServiceRest.UpdateInventoryBySkuAsync( inventoryToUpdate );

			Task.Delay( 2000 ).Wait();
			var getProductsTaskSoap = magentoServiceSoap.GetProductsAsync( new[] { 0, 1 }, skus : skus, includeDetails : false );
			getProductsTaskSoap.Wait();
			var updatedInventory = getProductsTaskSoap.Result.Where( p => p.ProductType == "simple" ).OrderBy( p => p.ProductId ).Select( p => new InventoryBySku { Sku = p.Sku, Qty = long.Parse( p.Qty ) } );
