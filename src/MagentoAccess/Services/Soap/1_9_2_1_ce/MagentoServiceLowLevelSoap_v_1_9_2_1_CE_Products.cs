...
{
	internal partial class MagentoServiceLowLevelSoap_v_1_9_2_1_ce : IMagentoServiceLowLevelSoap, IMagentoServiceLowLevelSoapGetProductsBySku
	{
			...
			try
			{
				Func< int, int, Func< int, string >, bool, bool, Task< List< SoapProduct > > > productsSelector = async ( start1, count1, selector1, removeSpecialSymbols, skipExceptions ) =>
				{
					try
					{
						...
						var productsResponses = await sourceList.ProcessInBatchAsync( this._getProductsMaxThreads, async x => await this.GetProductsPageAsync( productType, productTypeShouldBeExcluded, x, updatedFrom ).ConfigureAwait( false ) ).ConfigureAwait( false );
		...
		public virtual async Task< SoapGetProductsResponse > GetProductsAsync( string productType, bool productTypeShouldBeExcluded, DateTime? updatedFrom, IReadOnlyCollection< string > skus )
		{
			try
			{
		...
		protected virtual async Task< SoapGetProductsResponse > GetProductsPageAsync( string productType, bool productTypeShouldBeExcluded, string productIdLike, DateTime? updatedFrom )
		{
